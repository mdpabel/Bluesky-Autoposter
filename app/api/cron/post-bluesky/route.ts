...
export async function GET(req: NextRequest) {
  ...
  try {
    ...
    return NextResponse.json({ error: `RSS fetch failed: ${msg}` }, { status: 500 })
  }
  ...
  for (const item of items) {
    ...
    try {
      const postText = await generateBlueskyPost(item.title, item.contentSnippet, item.link)
  ...
  console.log(`[cron] Done — posted:${summary.posted} skipped:${summary.skipped} failed:${summary.failed}`)
